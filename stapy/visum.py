...
#
#
import dtapy.internals.jitclasses
...
try:
    ...
    def __get_network_data(visum: win32com.client.CDispatch):
        no = [int(val) for _, val in list(dtapy.internals.jitclasses.Links.GetMultiAttValues('No', True))]
        from_node = [int(val) for _, val in list(
            dtapy.internals.jitclasses.Links.GetMultiAttValues('FromNodeNo', True))]
        to_node = [int(val) for _, val in list(dtapy.internals.jitclasses.Links.GetMultiAttValues('ToNodeNo', True))]
        caps = [val for _, val in list(dtapy.internals.jitclasses.Links.GetMultiAttValues('CapPrt', True))]
        lengths = [val for _, val in list(dtapy.internals.jitclasses.Links.GetMultiAttValues('Length', True))]
        maxspeeds = [val for _, val in list(dtapy.internals.jitclasses.Links.GetMultiAttValues('V0PrT', True))]
        ...
        node_ids = [int(val) for _, val in list(dtapy.internals.jitclasses.Nodes.GetMultiAttValues('No', True))]
        node_xs_raw = [val for _, val in list(dtapy.internals.jitclasses.Nodes.GetMultiAttValues('XCoord', True))]
        node_ys_raw = [val for _, val in list(dtapy.internals.jitclasses.Nodes.GetMultiAttValues('YCoord', True))]
