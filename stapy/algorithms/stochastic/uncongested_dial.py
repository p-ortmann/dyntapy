...
def uncongested_stochastic_assignment(obj: StaticAssignment):
    ...
    for bush in obj.demand_dict:
        ...
        load_bush(bush, obj.edge_map, obj.inverse_edge_map, obj.link_ff_times, destinations, demands,
                  topological_orders[bush],
                  edges[bush], L[bush], flows, largest_destination_labels[bush])
...
# @njit
def load_bush(origin, edge_map, inverse_edge_map, costs, destinations, demands, topological_order, bush_edges, L, flows,
              largest_destination):
    ...
    edge_weights, node_weights = set_labels(origin, forward_star, backward_star, bush_edges, L, topological_order,
                                            largest_destination, inverse_edge_map, costs)
    node_flows = Dict()
    edge_flows = Dict()
    ...
    for j in topological_order[largest_destination::-1]:
        ...
        for index, _j in enumerate(destinations):
            if j == _j:
                destination_demand = demands[index]
        ...
        for i in backward_star[j]:
            edge_flows[(i,j)]=node_flows[j] * edge_weights[(i, j)] / node_weights[j]
...
def set_labels(origin, forward_star, backward_star, bush_edges, L, topological_order, largest_destination,
               inverse_edge_map, costs):
    edge_likelihood = dict()
    node_weights = dict()
    node_weights[origin] = 1
    edge_weights = dict()
    for edge in bush_edges:
        (i, j) = inverse_edge_map[edge]
        edge_likelihood[(i, j)] = exp(theta * (costs[edge] + L[i] + L[j]))
    for i in topological_order[:largest_destination]:
        if i != origin:
            node_weights[i] = 0
