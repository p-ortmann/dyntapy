...
def load_all_bushes(obj, topological_orders, edges, L, largest_destination_labels, costs, theta ):
    ...
    for bush in obj.demand_dict:
        ...
        if bush==6017:
            print('ola')
        forward_star=make_forward_stars(edges[bush], len(topological_orders[bush]))
        backward_star=make_backward_stars(edges[bush], len(topological_orders[bush]))
        ...
        flows=load_bush(bush, obj.edge_map, costs, destinations, demands,
                  ...
                  edges[bush], L[bush], flows, largest_destination_labels[bush], theta,forward_star, backward_star)
    return flows


#@njit
def generate_bushes(link_ff_times, edge_map, global_forward_star, demand_dict, node_order):
    ...
    for bush in demand_dict:
        ...
        L[bush], pred = __shortest_path(costs=link_ff_times, forward_star=global_forward_star, edge_map=edge_map, source=bush,
        ...
        #forward_stars[bush] = make_forward_stars(edges[bush], len(pred))
        #backward_stars[bush]= make_backward_stars(edges[bush], len(pred))
