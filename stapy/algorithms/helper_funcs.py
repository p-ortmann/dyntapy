...
def calculate_costs(link_capacities, link_ff_times, link_flows, method='bpr'):
    """ adds the weights on the edges of nx graph structure as travel time in accordance with BPR functions"""
    if method == 'bpr':
...
def aon(demand_dict, costs, forward_star, edge_map, number_of_od_pairs, node_order):
    ...
    for i in demand_dict:
        ...
        total_d = np.sum(demand_dict[i][1])
