...
def frank_wolfe(obj: __StaticAssignment):
    ...
    max_iterations = assignment_parameters['fw_max_iterations']
    delta = assignment_parameters['fw_delta']
    converged = False
    f1, f2 = np.zeros(obj.edge_order), np.zeros(obj.edge_order)  # initializing flow list with 0
    ...
    while not converged:
        ...
        _, f2 = aon(demand_dict=obj.demand_dict, costs=costs, forward_star=obj.forward_star, edge_map=obj.edge_map,
                    number_of_od_pairs=len(obj.od_flow_vector), node_order=obj.node_order)
        if counter > 1:
...
def __eval_obj_func_fw(state):
    bpr_beta = assignment_parameters['bpr_beta']
    bpr_alpha = assignment_parameters['bpr_alpha']
