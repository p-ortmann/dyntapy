...
def test_stress_dial_b():
    ...
    try:
        ...
        g = add_centroids(g, x, y, k=3, method="turn", name=names, place=place_tags)
    ...
    if method == "iterated":
        ...
        for seed in range(tot_seeds_to_try):
            ...
            json_demand = generate_od_xy(
                tot_od_pairs, city, seed=seed, max_flow=max_flow_per_od_pair
            )
            ...
            loading_failed, _, _ = loading(
                assignment.internal_demand, assignment.internal_network, results.flows
            )
            assert not loading_failed
            continuity_violations, _, _ = continuity(
                flows=results.flows, network=assignment.internal_network
            )
        ...
        json_demand = generate_od_xy(
            tot_od_pairs, city, seed=seed_constant, max_flow=max_flow_per_od_pair
        )
        od_graph = parse_demand(json_demand, g)
        assignment = StaticAssignment(g, od_graph)
        results = assignment.run(method="dial_b")
