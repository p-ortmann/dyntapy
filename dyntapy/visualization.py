...
default_plot_size = dynamic_parameters.visualization.plot_size
default_notebook_plot_size = dynamic_parameters.visualization.notebook_plot_size
default_max_links = dynamic_parameters.visualization.max_links
default_edge_width_scaling = dynamic_parameters.visualization.link_width_scaling
link_highlight_colors = dynamic_parameters.visualization.link_highlight_colors
node_highlight_color = dynamic_parameters.visualization.node_highlight_color
node_color = dynamic_parameters.visualization.node_color
centroid_color = dynamic_parameters.visualization.centroid_color
node_size = dynamic_parameters.visualization.node_size


def show_network(g: nx.MultiDiGraph,flows = None, link_kwargs=dict(), node_kwargs=dict(), highlight_links=np.array([]),
    ...
    if type(highlight_links) not in (np.ndarray, list):
        raise ValueError
    c, x, y = _get_colors_and_coords(tmp, max_width_coords, max_flow, flows, time_step=1,
    ...
    edge_renderer = plot.add_glyph(edge_source,
                                   glyph=Patches(xs='x', ys='y', fill_color='color', line_color="black",
                                                 line_alpha=0.8))
    edge_tooltips = [(item, f'@{item}') for item in dynamic_parameters.visualization.link_keys + list(link_kwargs.keys()) if
                     item != 'flow']
    if show_flows:
    ...
    if show_nodes:
        ...
        node_tooltips = [(item, f'@{item}') for item in dynamic_parameters.visualization.node_keys + list(node_kwargs.keys())]
...
def show_dynamic_network(g: nx.DiGraph, time: SimulationTime, flows=None, link_kwargs=dict(), node_kwargs=dict(),
    ...
    edge_tooltips = [(item, f'@{item}') for item in
                     dynamic_parameters.visualization.link_keys + list(link_kwargs.keys()) + list(static_link_kwargs.keys())
                     if
                     item != 'flow']
    ...
    if show_nodes:
        ...
        node_tooltips = [(item, f'@{item}') for item in
                         dynamic_parameters.visualization.node_keys + list(node_kwargs.keys()) + list(static_node_kwargs.keys())]
        node_hover = HoverTool(show_arrow=False, tooltips=node_tooltips, renderers=[node_renderer])
        url = "https://www.openstreetmap.org/node/@ext_id/"
        nodetaptool = TapTool(renderers=[node_renderer])
        nodetaptool.callback = OpenURL(url=url)
...
def _node_cds(g, highlight_nodes=np.array([]), **kwargs):
    visualization_keys = dynamic_parameters.visualization.node_keys
    node_dict = dict()
...
def _edge_cds(g, color, flow, x, y,step_size=1.0, **kwargs):
    visualization_keys = dynamic_parameters.visualization.link_keys
    edge_dict = dict()
