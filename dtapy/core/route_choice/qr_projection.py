            ...
            for link in links_to_check:
                ...
                for turn_id, (turn, out_link) in enumerate(zip(network.links.out_turns.get_nnz(link),
                    ...
                    if arrival <= min_cost:
                        ...
                        #gap = gap + min_cost * (cvn_down(t + 1, link, d) - cvn_down(t, link, d)) * \
                        #      turning_fractions[t, turn, d]
                    ...
                    gec_local = gec_local + (arrival - min_cost) * (
                            cvn_down[t + 1, link, d] - cvn_down[t, link, d]) * turning_fractions[t, turn, d]
                        ...
                        try:
                            for local_turn_id, turn in network.links.out_turns.get_row(link):
                                if local_short_turns[ptr] == local_turn_id:
                                    shift[turn, t] = shift[turn, t] + np.abs(sum_shift / local_short_turns.size)
                                    ...
                                    ptr += 1
                                    turning_fractions[t, turn, d] = turning_fractions[t, turn, d] + shift[turn, t]
                        except IndexError:
